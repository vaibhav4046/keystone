...
| 7 | Tests / CI green | Completed | the full suite passes (2 skipped) (`pytest -q`); GitHub Actions CI green incl. data.json drift check + Pages deploy (run 27893658209) | none |
| 8 | <3min demo video (HARD submission gate) | Waiting for user login or approval | An automated, self-narrated MP4 is now generated and committed: `SUBMISSION/keystone-demo.mp4` (~97s, ~2 MB) via `scripts/record_demo_video.mjs` (headless Chrome + ffmpeg), verified frame-by-frame. Full script also in `VIDEO_FINAL_READY.md` | Asset Completed; only the upload (YouTube/Vimeo unlisted) + pasting the URL into Devpost needs the user's login |

...
The **same engine is proven on real JavaScript**, not only Python: two committed, test-pinned JS graphs - **axios** (`isBuffer` x `isObject`, **62** shared dependents, all real axios functions) and **chalk** (`stringEncaseCRLFWithFirstIndex` x `stringReplaceAll`, 4). The JS pass also drops Array/String/Object builtin method names (`forEach`, `push`, `map`, ...) from call resolution, so a `.forEach()` is never mis-attributed to a repo function of the same name - which is what keeps the axios 62 honest (the naive pass reported a builtin-inflated 72). So Keystone is not an Orbit-Local-Python-only control: `scan-repo owner/repo` gates any public JS/TS or Python repo today.

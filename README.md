...
The ledger is a per-instance append-only file: one shared store per deployment, not a multi-tenant service, and a change id maps to a single symbol rather than a multi-symbol merge request (a real MR would take the union blast radius across touched symbols and apply the strictest tier; the gate already takes a blast set, so this is mainly an API-model extension). The per-minute approve rate limiter is a single-process convenience, not a production control; a multi-worker or multi-host deployment needs an external limit (a reverse proxy or the GitLab webhook rate limit). Precedent matches on the fully-qualified name when one is recorded and otherwise on the exact short symbol name, so a rename is a new identity and would not match a decision made under the old name; tying precedent to a stable symbol id is future work. Reviewer identity is self-asserted in the browser demo unless `KEYSTONE_APPROVE_TOKEN` is set; the GitLab CI gate path already binds the actor to a short-lived OIDC token's `sub` claim (`self_asserted=false`), and full offline RS256 verification against the issuer's JWKS is the remaining hardening step. The HMAC integrity key lives at `~/.keystone/ledger.key` (or `KEYSTONE_LEDGER_KEY`); back it up, because losing it makes an existing chain unverifiable, and rotating it re-keys only rows written after the change. The public deploy serves a committed real `orbit index` of this repo (status `SNAPSHOT`) with the per-symbol `orbit sql` command shown, rather than a live backend, because no free static host can run one. None of these are hidden in the UI; the status panel always says whether the data is LIVE, SNAPSHOT, or FALLBACK.

...
**▶ 90-second demo:** it autoplays in the hero on the live site, or watch it directly: https://vaibhav4046.github.io/keystone/keystone-demo.mp4 (two safe-looking MRs that break together → BLOCK → tamper-evident ledger).
...
python -m pytest -q                                   # the full suite passes (2 skipped)
...
Two more third-party graphs are committed and pinned the same way, so **every cited collision number recomputes on a committed artifact rather than living in a cache**: `benjaminp/six` (`_resolve` x `__get_module`, 3) and `psf/requests` (`values` x `set_cookie`, 48). `scripts/build_external_graphs.py` regenerates them and `tests/test_external_repo.py` pins them. Scanning is multi-language and uses a **real parser for every language**: Python via `ast`, and JavaScript/TypeScript via a **tree-sitter** AST (`core/repo_scan_ts.py`) - accurate def boundaries and call edges, correct on JSX, TS generics, decorators, and arrow functions, with call names read from real `call_expression` nodes (never from a comment, string, or regex). If tree-sitter is not installed the scanner falls back to a name-based regex pass, so the offline/zero-dependency path still works; the committed graphs are tree-sitter-built, so their pinned numbers reproduce on any machine.

The **same engine is proven on real JavaScript AND TypeScript**, not only Python, with the real tree-sitter parser: two committed, test-pinned graphs - **axios** (JS; `isObject` x `isBuffer`, **24** shared dependents, all real axios functions) and **zod** (TS; `clone` x `mergeDefs`, **39**, from 1,258 parser-extracted defs in a major TypeScript library). A `.forEach()` member call is never mis-attributed to a repo function of the same name (Array/String/Object builtin method names are excluded), and a JS/TS call resolves to a same-file def first before the global set, so a same-named helper in an unrelated module never fabricates a cross-file edge. So Keystone is not an Orbit-Local-Python-only control: `scan-repo owner/repo` gates any public Python, JS, or TS repo today, parsed by a real AST.
...
|-------|------------------------|
| The engine is deterministic and tested | `python -m pytest -q` → **the full suite passes (2 skipped)**. |
| The live engine answers (not just a static page) | `curl https://keystone-zt6c.onrender.com/api/proof` returns one payload: source mode, definitions, audit-chain state, integrity mode, the routes, the demo symbols, and the exact external-repo verify command. The landing also shows a live "Live backend verified" badge + proof panel read from `/api/status` on load. |
| There is a working demo video | `SUBMISSION/keystone-demo.mp4` (~97s) is generated by `scripts/record_demo_video.mjs` (headless Chrome + ffmpeg), self-narrated, re-runnable any time. |
...
- **Reviewer identity (cryptographic, no account needed):** a reviewer registers an **Ed25519 public key** in `.keystone/reviewers.json` and **signs each decision** (`python scripts/sign_decision.py`); `POST /api/approve` verifies the signature with an `X-Keystone-Signature` header against the registered key, so the recorded row carries `signature_verified=true`, `identity_source=ed25519-signature`, and `self_asserted=false` - WHO approved is proven by possession of a key (the signed-Git-commit / SSH / Sigstore model), and the proof is written into the tamper-evident ledger. A wrong signature is rejected (401), never silently downgraded. This needs nothing but local crypto - no GitLab account, no service. The **GitLab CI path** additionally binds the actor to an **OIDC id_token** `sub` claim (`core/identity.py`, `.gitlab-ci.yml`). Unsigned + no-token = honest self-asserted, as before.
...
3. **0:40 - The finding**: On the Command Center, read **SILENT COLLISION FOUND** (`compute_blast_radius` x `verify`, 5 shared dependents - the demo's narrative pair, a change to Keystone's own engine, deliberately chosen over the graph's single largest collision). Scroll to **All silent collisions - 16 found**: every colliding pair with its shared-dependent count (the largest on this self-index is `OrbitCliResult` x `_parse_rows`, 13), plus the full **safe merge order** - the headline number is inspectable, not a marquee.

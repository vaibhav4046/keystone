...
- Reviewer identity is self-asserted unless you set `KEYSTONE_APPROVE_TOKEN`, in which case `POST /api/approve` requires a matching `X-Keystone-Token` header (which proves possession, not identity). Because identity is self-asserted, the four-eyes (self-approval) check and agent gating are advisory labels in the demo, not enforced identity; binding them to a GitLab OIDC token whose `sub` claim becomes the recorded actor is the intended production path, and in CI the change author should come from `$CI_MERGE_REQUEST_AUTHOR`. A policy-BLOCK override is separately credentialed: when `KEYSTONE_OVERRIDE_TOKEN` is set, an override additionally requires a matching `X-Keystone-Override-Token`, so overriding is not the same privilege as approving. The README does not claim cryptographic identity it does not have.
